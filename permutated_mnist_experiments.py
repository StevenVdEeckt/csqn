...
parser = argparse.ArgumentParser(description='Permuted MNIST Baselines..')
...
parser.add_argument('--log_id', type=int, default=0, help="ID of log file, to assure it is unique. If -1, logging writes to Terminal")
...
init_alpha = {'EWC': 1000000, 'MAS': 10000, 'LWF': 1000}
...
if args.log_id == -1:
    ...
    logging.basicConfig(filename='perm_mnist_exp_%d_%d.log' % (n_tasks, args.log_id), level=logging.DEBUG, format=FORMAT, datefmt='%m/%d/%Y %I:%M:%S %p')
...
train_data1, _, _ = get_permuted_mnist_data(n_tasks=n_tasks, batch_size=1)
""" Load the network """
...
for method in run:
    ...
    for num in range(start_id[method], start_id[method] + iterations[method]):
        ...
        test_and_update(net_list, method + ' %d' % num if method != 'Fine-Tuning' else method)
